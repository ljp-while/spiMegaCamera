...
uint8_t channelData[]= {0x55, 0xff, 0x00, 0xaa};
...
void ArducamLink::cameraGetPicture(Arducam* myCamera)
{
	...
	uint8_t rtLength = 0;
	if (cameraInstance->csPin == CS1_PIN)
	{
		channelData[2] = 0x00;
    	arducamUartWriteBuff(channelData,sizeof(channelData));
	}
	else if (cameraInstance->csPin == CS2_PIN)
	{
		channelData[2] = 0x01;
    	arducamUartWriteBuff(channelData,sizeof(channelData));
	}
	else if (cameraInstance->csPin == CS3_PIN)
	{
		channelData[2] = 0x02;
    	arducamUartWriteBuff(channelData,sizeof(channelData));
	}
	else if (cameraInstance->csPin == CS4_PIN)
	{
		channelData[2] = 0x03;
    	arducamUartWriteBuff(channelData,sizeof(channelData));
	}	
...
uint8_t ArducamLink::uartCommandProcessing(Arducam* myCAM,uint8_t* commandBuff)
{
	...
	case SET_VIDEO_RESOLUTION:																//Set Video Resolution
		cameraResolution = commandBuff[1] & 0x0f;
		myCAM->takePicture((CAM_IMAGE_MODE)cameraResolution,CAM_IMAGE_PIX_FMT_JPG);
